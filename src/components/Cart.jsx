...
function Cart() {
  ...
  return (
    <aside className="w-full sm:w-2/3 lg:w-1/3 flex flex-col items-center px-3">
        <div className="w-full bg-white shadow-lg border-2 rounded-lg flex flex-col p-4">
            <div className='flex justify-between items-center pb-5'>
              <p className="text-xl font-semibold">ID Cart: {userCart.id}</p>
            </div>
            <p className="pb-2">Subtotal: {formatMoney(total)}</p>
            <hr className='my-2' />
            <p className="pb-2">Total: {formatMoney(total)}</p>
            <button 
              type='submit'
